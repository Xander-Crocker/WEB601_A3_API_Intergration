...
router.delete(
    ...
    async (req, res, next) =>{
        try {    
            ...
            await axiosRequest.delete(`/shops/${shop}/products/${id}.json`).then((response) => {
                // console.log(response.data);
                return res.status(200).json({message: "Product deleted successfully"});
            }).catch((error) => {
                console.log(error);
                return res.status(400).json({ error: "The product was unable to be deleted. Please try again."});
            });
        } catch (error){
